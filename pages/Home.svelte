...
<img src="./images/{setting.top_image}" 
     ...
     class="logo-img">
<!--MESSAGE M1 -->
<div class="mdc-typography--headline4 shop-title">{message.msg1}</div>
<div class="mdc-typography--subtitle2 shop-title">{message.msg2}</div>
...
<script>
   ...
   // MATERIAL ICONS - SVG
	import { AddCircle, ArrowDownward, Cancel, CancelPresentation, MoreVert,
	         RemoveCircle, ShoppingCart
	       } from '../material-icons/_index';
  ...
  // --------------------------------------------------------------------
   ...
   function openCart(n){
      ...
      // alert("OKpack[i]:"+data_buff.length);
      // alert("OK:_products.length:"+_products.length);
...
.selected-text{
  ...
  .card__primary-mbl{
   ...
   .chip-gram{
      ...
      height: 30px;
      color: #558b2f;
      background-color: #c5e1a5;/*#4fc3f7*/
      display: flex;
